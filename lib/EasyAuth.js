modules.export = (function(){
  ...
  function user(callback){
    ...
    var self = this;
    read({prompt: this.options.userPrompt, default: process.env.USER || process.env.LOGNAME}, function(err, user){
  ...
  function pass(callback){
    var self = this;
    read({prompt: this.options.passPrompt, silent: true}, function(err, pass){
      if (err) self.emit('error', err);
      else self.pass = pass;
      callback();
    });
